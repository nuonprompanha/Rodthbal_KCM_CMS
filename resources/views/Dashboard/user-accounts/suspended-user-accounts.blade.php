...
@section('content')
    <main class="app-main">
        ...
        <div class="app-content-header">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <h3 class="mb-0"> Suspended Users Accounts</h3>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-end">
                            <li class="breadcrumb-item"><a href="#">Home</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Suspended Users Accounts</li>
                        </ol>
                    </div>
                </div>
                <!--end::Row-->
            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title m-0 p-0"><i class="fa-solid fa-list-ul"></i> Suspended Users Accounts</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @forelse($suspendedUsers as $index => $user)
                                        <tr>
                                            <td class="text-center align-middle">{{ $suspendedUsers->firstItem() + $index }}
                                            </td>
                                            ...
                                            <td class="align-middle text-center">
                                                <button type="button" class="btn btn-success btn-sm btn-restore-user"
            ...
            <div class="modal-content">
                <form action="{{ route('dashboard.user-accounts.store') }}" method="POST">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="createUserModalLabel"><i class="fa-solid fa-user-plus me-2"></i>Create
                            User</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('address') is-invalid @enderror" id="createUserAddress"
                                name="address" rows="2" placeholder="Address">{{ old('address') }}</textarea>
                            @error('address')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <form id="modifyUserForm" method="POST" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="modifyAddress" name="address" rows="2"
                                placeholder="Address">{{ old('address', $modifyUser?->address) }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label for="modifyDepartment" class="form-label">Department <span
                                    class="text-danger">*</span></label>
                            <select class="form-select" id="modifyDepartment" name="user_department_id" required>
                                <option value="">— Select Department —</option>
                                @foreach ($departments as $dept)
    ...
    <!-- Hidden form for restore (submitted via JS after SweetAlert2 confirm) -->
    <form id="restoreUserForm" method="POST" action="" class="d-none"
        data-action-template="{{ route('dashboard.user-accounts.restore', ['user' => '__ID__']) }}">
        @csrf
    </form>

    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Restore user: SweetAlert2 confirm then submit form
            var restoreForm = document.getElementById('restoreUserForm');
            document.querySelectorAll('.btn-restore-user').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    var userId = this.getAttribute('data-user-id');
                    var userName = this.getAttribute('data-user-name') || 'this user';
                    var userEmail = this.getAttribute('data-user-email') || '';
                    var actionTemplate = restoreForm && restoreForm.getAttribute('data-action-template');
                    var actionUrl = actionTemplate ? actionTemplate.replace('__ID__', userId) : '{{ url('dashboard/user-accounts') }}/' + userId + '/restore';

                    Swal.fire({
                        title: 'Restore user?',
                        html: 'Restore <strong>' + userName + '</strong>' +
                            (userEmail ? ' <span class="text-muted small">(' + userEmail + ')</span>' : '') +
                            '<br><br>They will be able to log in again.',
                        icon: 'question',
                        ...
                        if (result.isConfirmed && restoreForm) {
